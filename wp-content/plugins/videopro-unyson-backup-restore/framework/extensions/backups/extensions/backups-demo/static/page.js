	...
	var inst = {
		...
		init: function(){
			...
			$('#fw-ext-backups-demo-list').on('click', '[data-install]', function(){
				...
				$( '#videopro-dialog-confirm' ).css( 'dispplay', 'block' );

				if ( empty ) {
					$( "#videopro-dialog-confirm" ).dialog({
					      resizable: false,
					      height: "auto",
					      width: 400,
					      modal: true,
					      open: function(){
						  	$('#videopro-dialog-content').html( confirm_message );
						  },
					      buttons: {
					      	"Install Demo": function() {
					          $( this ).dialog( "close" );
					          $( '#videopro-dialog-confirm' ).css( 'dispplay', 'none' );
